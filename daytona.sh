...
# ══════════════════════════════════════════════════════════════
run_qemu() {
  # $1 = RAM, $2 = CPU cores, $3 = SSH port, $4 = boot order, $5 = optional ISO
  local ram="$1" cpus="$2" port="$3" boot="$4" iso="${5:-}"
  local iso_args=() seed_args=()

  if [ -n "$iso" ] && [ -f "$iso" ]; then
    iso_args=(-device virtio-scsi-pci,id=scsi0
              -device scsi-cd,drive=cd0
              -drive "id=cd0,if=none,format=raw,file=$iso,readonly=on")
  fi
  if [ -f "$VM_SEED" ] && [ "$boot" = "d" ]; then
    seed_args=(-drive "file=$VM_SEED,format=raw,if=virtio")
  fi
  ...
  if [ -e /dev/kvm ] && [ -r /dev/kvm ] && [ -w /dev/kvm ]; then
    ...
    if qemu-system-x86_64 -enable-kvm -cpu host "${common_args[@]}"; then
      return 0
    fi
    msg_warn "KVM failed; falling back to TCG / KVM lỗi; chuyển sang TCG"
  else
    msg_warn "KVM unavailable; using TCG / KVM không khả dụng; dùng TCG"
  fi

  qemu-system-x86_64 -accel tcg,thread=multi -cpu max "${common_args[@]}"
}
